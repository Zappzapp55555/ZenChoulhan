...
// Déclarées ici (tôt) car quizStartPrefetch() est appelée dès le démarrage de l'app, juste après
// history.replaceState ci-dessous. Si ces "let" restaient déclarées plus loin dans le fichier
// (près d'initQuizList), l'appel précoce à quizStartPrefetch() plantait avec une ReferenceError
// ("Cannot access before initialization" — zone morte temporelle), ce qui interrompait TOUTE la
// suite du script et cassait l'application entière au lancement.
let quizPrefetchResult = null;
...
// Dès l'ouverture de l'app, on lance déjà en arrière-plan la génération du premier quiz Gemini,
// pour qu'il soit prêt (ou presque) avant même que l'utilisateur touche à l'écran Quiz. On ne le
// fait que si l'app est déjà déverrouillée (voir l'écran de verrouillage tout en haut du body) :
// pas question de consommer du quota Gemini pour un visiteur qui n'a pas encore entré le code.
...
const QUIZ_CHAPTERS = [
  {
    ...
    subtitle: "10 questions générées par IA à chaque lancement (secours local si indisponible)",
    questions: [
      {
        id: "q1",
      ...
      {
        id: "q39",
        scenario: "Un moustique ou une mouche vole librement dans la pièce. Tu retournes un verre par-dessus pour le piéger dessous, sans l'écraser.",
        options: ["Permis", "Interdit"],
        correctIndex: 1,
        badge: "interdit",
        level: "deoraita",
        melakha: "Tsad (chasser/capturer)",
        source: "Choulhan Aroukh, Orah Haim, siman 316",
        explain: "Capturer une créature qui se déplaçait librement, même sans lui faire aucun mal, est le travail de « chasser » (צד), l'un des 39 travaux, interdit d'origine biblique (deoraita) — le seul fait de restreindre sa liberté de mouvement suffit.",
        howto: "Chasse l'insecte dehors en ouvrant une fenêtre ou une porte plutôt que de le piéger sous un verre. Si un insecte est dangereux (guêpe, etc.), les règles s'assouplissent — demande à ton rav en cas de doute."
      },
      {
        id: "q40",
        scenario: "Tu découpes une feuille de papier aluminium ou de papier cuisson à la taille exacte du plat, en suivant une mesure précise, pour l'utiliser plus tard dans la semaine.",
        options: ["Permis", "Interdit"],
        correctIndex: 1,
        badge: "interdit",
        level: "derabbanan",
        melakha: "Mékhaték (découper à une mesure précise)",
        source: "Choulhan Aroukh, Orah Haim, siman 322",
        explain: "Découper un matériau à une dimension exacte et réfléchie, en vue d'un usage futur précis, est rattaché au travail de « mékhaték » — un interdit d'origine rabbinique (derabbanan) pour ce type de découpe soignée, à la différence de déchirer grossièrement un emballage pour un besoin immédiat.",
        howto: "Découpe tes feuilles de papier aluminium ou de cuisson avant l'entrée de Chabbat si tu sais que tu en auras besoin à une taille précise. Une découpe approximative, pour un usage immédiat, pose beaucoup moins de problème."
      },
      {
        id: "q41",
        scenario: "Tu coupes des légumes en tout petits dés avec un couteau pour préparer une salade que tu vas manger tout de suite.",
        options: ["Permis", "Interdit"],
        correctIndex: 0,
        badge: "permis",
        level: "derabbanan",
        melakha: "Tokhén (moudre, exception pour la consommation immédiate)",
        source: "Choulhan Aroukh, Orah Haim, siman 321",
        explain: "Couper des légumes, même très finement, avec un couteau normal, pour une consommation immédiate, est considéré comme la façon habituelle de préparer un repas — ce n'est pas assimilé au travail de « moudre » (טוחן), contrairement au fait de moudre une épice avec un moulin.",
        howto: "Coupe librement tes légumes au couteau pour le repas en cours. Évite en revanche une râpe fine ou un robot mixeur qui réduit les aliments en une texture proche de la purée ou de la poudre."
      },
      {
        id: "q42",
        scenario: "Tu démontes une étagère de rangement en plastique, pièce par pièce, pour la ranger, sans intention de la remonter avant la fin de Chabbat.",
        options: ["Permis", "Interdit"],
        correctIndex: 1,
        badge: "interdit",
        level: "deoraita",
        melakha: "Sotér (démolir)",
        source: "Choulhan Aroukh, Orah Haim, siman 314",
        explain: "Démonter une structure assemblée, même simple et réversible, est le travail de « démolir » (סותר), un dérivé direct de construire (בונה) — un interdit d'origine biblique (deoraita), même quand il n'y a aucune destruction définitive.",
        howto: "Démonte ce type de meuble ou d'étagère avant l'entrée de Chabbat ou après sa sortie, pas pendant Chabbat lui-même."
      },
      {
        id: "q43",
        scenario: "Tu utilises du correcteur liquide (blanco) ou tu grattes avec un objet pointu pour effacer un mot mal écrit sur un document, dans l'idée d'écrire autre chose à la place.",
        options: ["Permis", "Interdit"],
        correctIndex: 1,
        badge: "interdit",
        level: "deoraita",
        melakha: "Mokhék (effacer)",
        source: "Choulhan Aroukh, Orah Haim, siman 340",
        explain: "Effacer une écriture dans le but d'écrire à nouveau à cet endroit est le travail de « effacer » (מוחק), l'un des 39 travaux, interdit d'origine biblique (deoraita) — le pendant symétrique de l'écriture elle-même.",
        howto: "Corrige et prépare tes documents avant l'entrée de Chabbat. Pendant Chabbat, laisse une erreur telle quelle plutôt que de l'effacer."
      },
      {
        id: "q44",
        scenario: "Un reste de kougel entièrement cuit, solide et sans sauce, avait été retiré du blech et posé sur le plan de travail. Tu le reposes directement sur le blech pour le réchauffer.",
        options: ["Permis", "Interdit"],
        correctIndex: 0,
        badge: "permis",
        level: "derabbanan",
        melakha: "Bishoul (cuire — exception pour un aliment sec déjà cuit)",
        source: "Choulhan Aroukh, Orah Haim, siman 253:2",
        explain: "Contrairement à un plat en sauce ou liquide, un aliment totalement sec et déjà bien cuit ne pose pas le même risque de « cuisson après cuisson » : les conditions strictes de la ha'hazara (tenir en main, ne pas poser) sont assouplies pour ce cas, tant que la plaque reste couverte.",
        howto: "Cette souplesse ne vaut que pour un aliment sec et déjà complètement cuit. Dès qu'il y a une sauce, un liquide, ou un doute sur la cuisson complète, reviens aux règles strictes de la ha'hazara (voir la question sur la casserole)."
      },
      {
        id: "q45",
        scenario: "Tu presses des olives pour en extraire de l'huile.",
        options: ["Permis", "Interdit"],
        correctIndex: 1,
        badge: "interdit",
        level: "deoraita",
        melakha: "Dash (extraire un liquide — cas biblique)",
        source: "Choulhan Aroukh, Orah Haim, siman 320",
        explain: "Contrairement au citron ou à d'autres fruits, l'olive et le raisin sont les deux cas classiques où extraire le liquide (ici l'huile) est un interdit d'origine biblique (deoraita) à part entière, car ce sont les fruits pour lesquels ce travail était pratiqué à l'époque du Michkan.",
        howto: "Utilise uniquement de l'huile d'olive déjà extraite avant Chabbat. Ne presse jamais d'olives ou de raisins toi-même le jour de Chabbat, contrairement au cas plus souple du citron sur un aliment déjà présent."
      },
      {
        id: "q46",
        scenario: "En mangeant du poisson, tu retires les arêtes avec les doigts ou avec ta fourchette, juste avant d'avaler la bouchée, pour ne garder que la chair.",
        options: ["Permis", "Interdit"],
        correctIndex: 0,
        badge: "permis",
        level: "derabbanan",
        melakha: "Borer (trier — exception pour manger immédiatement)",
        source: "Choulhan Aroukh, Orah Haim, siman 319",
        explain: "Séparer le déchet (l'arête) de ce qui va être mangé, à la main, juste avant de consommer, est la façon normale de manger et n'est pas considéré comme le travail de trier (בורר) — contrairement à trier un mélange pour le mettre de côté en vue de plus tard.",
        howto: "Retire les arêtes ou les peaux juste avant chaque bouchée. Évite de désarêter tout le poisson d'avance pour toute la table si ce n'est pas pour une consommation immédiate."
      },
      {
        id: "q47",
        scenario: "Tu mélanges des flocons d'avoine avec du lait, en remuant vigoureusement jusqu'à obtenir un porridge épais et homogène.",
        options: ["Permis", "Interdit"],
        correctIndex: 1,
        badge: "interdit",
        level: "derabbanan",
        melakha: "Lash (pétrir, appliqué à un aliment)",
        source: "Choulhan Aroukh, Orah Haim, siman 321",
        explain: "Lier des particules solides avec un liquide jusqu'à former une pâte homogène et épaisse est rattaché au travail de « pétrir » (לש), même appliqué à des aliments plutôt qu'à de la farine à pain. L'usage suivi (notamment achkénaze, d'après le Rama) traite cela comme un interdit rabbinique (derabbanan) bien réel.",
        howto: "Prépare ce type de porridge ou de mélange épais avant l'entrée de Chabbat, ou verse le lait sans trop remuer pour éviter d'obtenir une pâte bien liée et homogène."
      },
      {
        id: "q48",
        scenario: "Tu remplaces la pile d'une montre à quartz ou d'une télécommande en panne pour la remettre en état de marche.",
        options: ["Permis", "Interdit"],
        correctIndex: 1,
        badge: "interdit",
        level: "derabbanan",
        melakha: "Makéh Bépatich (donner le coup de fin, achever un objet)",
        source: "Choulhan Aroukh, Orah Haim, siman 313",
        explain: "Rendre à nouveau fonctionnel un appareil qui ne marchait plus est rattaché au principe de « donner le coup de fin » (מכה בפטיש) — achever ou réparer un objet pour le rendre utilisable. Les décisionnaires contemporains traitent le remplacement de pile comme un interdit rabbinique (derabbanan) dans ce cadre, en plus de toute question électrique propre à l'appareil.",
        howto: "Change les piles de tes appareils avant l'entrée de Chabbat si tu penses en avoir besoin. Pendant Chabbat, laisse l'appareil éteint plutôt que de le réparer."
      },
      {
        id: "q49",
        scenario: "Tu prépares et dresses la table pour le repas de sortie de Chabbat (Melavé Malka), dès le début de l'après-midi, en plein Chabbat.",
        options: ["Permis", "Interdit"],
        correctIndex: 1,
        badge: "interdit",
        level: "derabbanan",
        melakha: "Hachana (préparer pour après Chabbat)",
        source: "Choulhan Aroukh, Orah Haim, siman 323",
        explain: "Préparer activement quelque chose qui ne sert qu'après la sortie de Chabbat, sans aucun besoin pendant Chabbat lui-même, est un interdit rabbinique (derabbanan) de « hachana » — contrairement à laver une vaisselle dont on a un vrai besoin avant la fin du Chabbat.",
        howto: "Attends la sortie de Chabbat (après Havdala) pour dresser la table du repas suivant, même si tu sais déjà ce que tu vas servir."
      },
      {
        id: "q50",
        scenario: "Une page de livre s'est déchirée. Tu la répares avec du ruban adhésif, pour qu'elle tienne de façon durable.",
        options: ["Permis", "Interdit"],
        correctIndex: 1,
        badge: "interdit",
        level: "derabbanan",
        melakha: "Tofer (coudre, par extension au collage)",
        source: "Choulhan Aroukh, Orah Haim, siman 340",
        explain: "Assembler durablement deux morceaux de papier ou de tissu avec de la colle ou du ruban adhésif est rattaché par les décisionnaires au même principe que coudre (תופר) : créer une jonction stable entre deux éléments séparés. C'est classé comme un interdit rabbinique (derabbanan) pour ce type de réparation improvisée.",
        howto: "Répare les pages abîmées avant l'entrée de Chabbat. Pendant Chabbat, tu peux glisser un signet ou refermer le livre avec précaution sans réparer la déchirure."
      },
      {
        id: "q51",
        scenario: "Tu assembles pour la première fois les éléments neufs d'une étagère en kit, en les emboîtant et en les vissant ensemble.",
        options: ["Permis", "Interdit"],
        correctIndex: 1,
        badge: "interdit",
        level: "deoraita",
        melakha: "Boneh (construire)",
        source: "Choulhan Aroukh, Orah Haim, siman 313",
        explain: "Assembler des pièces séparées pour créer un objet fonctionnel neuf est le travail de « construire » (בונה), l'un des 39 travaux, interdit d'origine biblique (deoraita) — à l'inverse du cas d'une clé tournée dans une serrure déjà existante, qui n'est qu'un usage normal d'un objet déjà achevé.",
        howto: "Monte ce type de meuble en kit avant l'entrée de Chabbat ou après sa sortie, jamais pendant Chabbat."
      },
      {
        id: "q52",
        scenario: "Tu appliques du rouge à lèvres ou du vernis à ongles avant de sortir.",
        options: ["Permis", "Interdit"],
        correctIndex: 1,
        badge: "interdit",
        level: "deoraita",
        melakha: "Tsové'a (teindre)",
        source: "Choulhan Aroukh, Orah Haim, siman 303",
        explain: "Colorer durablement une surface, y compris la peau, les lèvres ou les ongles, est le travail de « teindre » (צובע), l'un des 39 travaux, interdit d'origine biblique (deoraita) selon la majorité des décisionnaires appliqué au maquillage.",
        howto: "Maquille-toi et fais-toi les ongles avant l'entrée de Chabbat si tu penses en avoir besoin pour le Chabbat lui-même."
      },
      {
        id: "q53",
        scenario: "Tu donnes des croquettes à ton chien depuis un sachet déjà ouvert avant Chabbat, comme chaque jour.",
        options: ["Permis", "Interdit"],
        correctIndex: 0,
        badge: "permis",
        level: "derabbanan",
        melakha: "Muktsé (la nourriture n'est jamais muktsée)",
        source: "Choulhan Aroukh, Orah Haim, siman 324",
        explain: "Un aliment, y compris pour un animal, a toujours un usage permis (le nourrir) : il n'est donc jamais muktsé, contrairement à un objet comme de l'argent ou un téléphone qui n'a aucun usage permis le Chabbat. On a de plus une obligation de nourrir les animaux dont on a la responsabilité.",
        howto: "Sers la nourriture depuis un contenant déjà ouvert avant Chabbat ; évite seulement d'ouvrir un nouvel emballage scellé (voir la question sur le pot)."
      },
      {
        id: "q54",
        scenario: "Tu consultes des annonces immobilières ou tu discutes du prix d'un bien que tu comptes acheter, comme tu le ferais en semaine.",
        options: ["Permis", "Interdit"],
        correctIndex: 1,
        badge: "interdit",
        level: "derabbanan",
        melakha: "Ouvdin Déḥol (activité de type profane)",
        source: "Choulhan Aroukh, Orah Haim, siman 307",
        explain: "Discuter ou s'occuper activement d'affaires commerciales et de calculs financiers de la semaine, même sans écrire ni conclure de transaction, est un interdit rabbinique (derabbanan) : Chabbat doit rester à l'écart des préoccupations profanes du quotidien.",
        howto: "Repousse ces recherches et discussions après la sortie de Chabbat. Un simple échange qui n'a pas un caractère de calcul ou de tractation commerciale reste permis."
      },
      {
        id: "q55",
        scenario: "Tu déplaces un simple caillou du jardin, sans aucune raison particulière.",
        options: ["Permis", "Interdit"],
        correctIndex: 1,
        badge: "interdit",
        level: "derabbanan",
        melakha: "Muktsé machmat goufo (objet sans aucun usage)",
        source: "Choulhan Aroukh, Orah Haim, siman 308",
        explain: "Un objet qui n'a par nature aucun usage permis le Chabbat (comme une pierre) est muktsé « machmat goufo » (à cause de lui-même) : il ne peut être déplacé que pour un besoin réel de son emplacement, pas sans raison.",
        howto: "Laisse les pierres et objets similaires où ils sont, sauf besoin réel de dégager la place qu'ils occupent."
      },
      {
        id: "q56",
        scenario: "Ton chat, qui vit à la maison en permanence, se faufile dans une pièce fermée ; tu le rattrapes simplement pour le ramener dans le salon.",
        options: ["Permis", "Interdit"],
        correctIndex: 0,
        badge: "permis",
        level: "derabbanan",
        melakha: "Tsad (chasser/capturer — exception pour un animal déjà domestiqué)",
        source: "Choulhan Aroukh, Orah Haim, siman 316",
        explain: "Le travail de « chasser » (צד) concerne une créature véritablement en liberté. Un animal domestique qui vit en permanence dans la maison et dépend de son propriétaire n'est pas considéré comme « en liberté » : le rattraper pour le ramener n'est donc pas un cas de Tsad, contrairement à un insecte qui vole librement.",
        howto: "Tu peux rattraper et ramener un animal domestique habitué à la maison sans crainte, à la différence d'un animal ou d'un insecte qui se déplace librement à l'état sauvage."
      },
      {
        id: "q57",
        scenario: "Une pomme tombe toute seule de l'arbre du jardin pendant que c'est déjà Chabbat, sans que personne ne l'ait cueillie.",
        options: ["Permis", "Interdit"],
        correctIndex: 1,
        badge: "interdit",
        level: "derabbanan",
        melakha: "Nolad (un objet « né » pendant Chabbat)",
        source: "Choulhan Aroukh, Orah Haim, siman 322",
        explain: "Un fruit qui se détache et devient utilisable seulement pendant Chabbat lui-même n'était pas prêt (« mouzman ») avant l'entrée de Chabbat : il est muktsé ce jour-là au titre de « nolad », un interdit rabbinique (derabbanan), même s'il redevient permis dès le lendemain.",
        howto: "Laisse le fruit tombé au sol jusqu'à la sortie de Chabbat. Seul un fruit déjà détaché avant l'entrée de Chabbat peut être ramassé et mangé normalement."
      },
      {
        id: "q58",
        scenario: "Tu prends une douche chaude complète, comme n'importe quel autre jour de la semaine.",
        options: ["Permis", "Interdit"],
        correctIndex: 1,
        badge: "interdit",
        level: "derabbanan",
        melakha: "Réchitsa (décret rabbinique sur le bain chaud)",
        source: "Choulhan Aroukh, Orah Haim, siman 326",
        explain: "Nos sages ont interdit de se laver tout le corps à l'eau chaude le Chabbat, même si l'eau a été chauffée avant Chabbat, de peur qu'on en vienne à chauffer de l'eau soi-même ce jour-là. C'est un interdit rabbinique (derabbanan), distinct de la cuisson elle-même.",
        howto: "Douche-toi avant l'entrée de Chabbat. Pendant Chabbat, une toilette partielle à l'eau tiède ou froide reste généralement plus simple à permettre — demande à ton rav les détails selon ta communauté."
      },
      {
        id: "q59",
        scenario: "Un peu d'eau se renverse sur la table pendant le repas. Tu l'essuies avec une éponge ou un chiffon bien absorbant.",
        options: ["Permis", "Interdit"],
        correctIndex: 1,
        badge: "interdit",
        level: "deoraita",
        melakha: "Sehita (essorer, par conséquence certaine)",
        source: "Choulhan Aroukh, Orah Haim, siman 301",
        explain: "Essuyer un liquide avec un matériau conçu pour absorber entraîne presque inévitablement, tôt ou tard, le fait de le presser ou de l'essorer — une conséquence certaine (« pessik reisha ») de l'interdit d'essorer, rattaché comme un dérivé du travail de lessive (deoraita), même si ce n'est pas ton intention première.",
        howto: "Repousse l'eau avec une raclette ou un objet non absorbant, ou utilise un papier essuie-tout jetable en tapotant sans jamais presser ni essorer."
      },
      {
        id: "q60",
        scenario: "Une assiette avec de la viande déjà bien cuite et complètement sèche sort du réfrigérateur, avec quelques gouttes d'eau de condensation au fond. Tu la poses directement sur la plata sans l'essuyer.",
        options: ["Permis", "Interdit"],
        correctIndex: 1,
        badge: "interdit",
        level: "deoraita",
        melakha: "Bishoul (cuire un liquide froid)",
        source: "Choulhan Aroukh, Orah Haim, siman 253",
        explain: "Les gouttes d'eau de condensation ont le statut d'un liquide froid à part entière. Les réchauffer directement sur la plata revient à les cuire à nouveau, un interdit d'origine biblique (deoraita) — même si l'aliment solide qui les porte est, lui, déjà entièrement cuit.",
        howto: "Essuie soigneusement le fond de l'assiette avec un papier absorbant avant de la poser sur la plata, chaque fois qu'elle sort du réfrigérateur.",
        modernView: "Certains décisionnaires considèrent que ces quelques gouttes de condensation sont annulées (bâtel) dans la masse de l'aliment solide et qu'on ne recherche de toute façon pas à les cuire : ils permettent de reposer l'assiette sans l'essuyer, du moment que l'aliment lui-même est déjà entièrement cuit et sec.",
        modernViewAttribution: "Rapporté au nom de Rav Shlomo Zalman Auerbach (cité dans Chemirat Chabbat Kehilkhatah) — à vérifier avec ton rav, cette position n'est pas unanime."
      },
      {
        id: "q61",
        scenario: "Une bougie allumée sur la table commence à faire couler de la cire sur la nappe. Tu déplaces le bougeoir pour éviter d'abîmer la nappe, sans aucun risque réel d'incendie.",
        options: ["Permis", "Interdit"],
        correctIndex: 1,
        badge: "interdit",
        level: "derabbanan",
        melakha: "Muktsé (bassis lédavar ha'assour)",
        source: "Choulhan Aroukh, Orah Haim, siman 310",
        explain: "Le bougeoir qui portait une flamme allumée dès l'entrée de Chabbat devient un support d'objet muktsé et reste muktsé toute la journée. Ce statut rigoureux s'applique même pour protéger un bien matériel (une nappe) — seul un vrai danger justifierait de le déplacer.",
        howto: "Place une protection (plateau en métal ou en verre) sous tes bougies avant l'entrée de Chabbat. Seul un risque réel d'incendie autoriserait à déplacer ou éteindre la bougie pendant Chabbat."
      },
      {
        id: "q62",
        scenario: "En mangeant un os de poulet, tu le suces directement avec la bouche pour en extraire un peu de jus ou de moelle.",
        options: ["Permis", "Interdit"],
        correctIndex: 0,
        badge: "permis",
        level: "deoraita",
        melakha: "Mefarek / Borer (extraire — exception pour le mode normal de manger)",
        source: "Choulhan Aroukh, Orah Haim, siman 319",
        explain: "Extraire un aliment directement avec la bouche, pendant l'acte même de manger, fait partie du mode normal de consommation (« derekh akhila ») et échappe aux interdits de trier ou d'extraire, qui concernent les manipulations faites à la main ou avec un ustensile.",
        howto: "Cette souplesse concerne l'extraction directe à la bouche pendant le repas ; utiliser un instrument pour extraire la moelle en dehors de l'acte de manger serait plus problématique."
      }
    ]
  }
];
...
// Génération des questions du quiz via Gemini, à chaque lancement. Isolée : ne touche jamais
// QUIZ_CHAPTERS (la banque fixe vérifiée à la main), qui ne sert que de secours si l'IA échoue
// (quota, panne réseau, clé API manquante) pour que le quiz reste toujours utilisable.
async function quizGenerateAIQuestions(){
  const apiKey = localStorage.getItem("geminiApiKey");
  if (!apiKey) {
      ...
      questions: {
        ...
        items: {
          ...
          properties: {
            ...
            modernView: { type: "STRING", description: "OPTIONNEL, laisser une chaîne vide '' dans l'immense majorité des cas. À remplir UNIQUEMENT si tu connais un vrai débat documenté entre décisionnaires sur ce cas précis, où un avis reconnu est plus souple que la réponse donnée ci-dessus. Ne jamais inventer un débat qui n'existe pas juste pour remplir ce champ." },
            modernViewAttribution: { type: "STRING", description: "OBLIGATOIRE si modernView est rempli, sinon laisser vide. Doit nommer un décisionnaire ou une source précise et réelle (ex: 'Rav Ovadia Yosef, Yalkout Yossef' ou 'Rav Moshe Feinstein, Igrot Moshe'). Interdiction absolue d'écrire une formule vague comme 'certains décisionnaires' ou 'des rabbins modernes' sans nom : si tu ne peux pas nommer une source précise et réelle, laisse modernView ET modernViewAttribution vides." }
          },
          required: ["scenario", "correctIndex", "badge", "level", "melakha", "source", "explain", "howto", "modernView", "modernViewAttribution"]
        }
      }
    },
    required: ["questions"]
  ...
  const promptText = `
    ...
    Génère un questionnaire de strictement 10 questions sur les Halakhot de Chabbat.
    ...
    CONTRAINTES SUR "modernView" ET "modernViewAttribution" :
    - Laisse ces deux champs vides ('') pour la quasi-totalité des questions.
    - Remplis-les uniquement si tu connais un vrai débat reconnu entre décisionnaires sur ce cas précis, où un avis plus souple existe.
    - Si tu remplis modernView, tu DOIS nommer précisément un décisionnaire ou un ouvrage réel dans modernViewAttribution (ex: "Rav Ovadia Yosef, Yalkout Yossef"). N'écris jamais une formule vague comme "certains décisionnaires" sans nom : dans ce cas, laisse les deux champs vides plutôt que d'inventer ou de généraliser.
  `;
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    try {
      ...
      if (response.status === 429) {
        ...
        return { success: false, error: "Limite de requêtes Gemini atteinte (429)." };
      }
      ...
      parsedData.questions.forEach((q, index) => {
        ...
        finalQuestions.push({
          ...
          modernView: (typeof q.modernView === "string" && typeof q.modernViewAttribution === "string" && q.modernView.trim() && q.modernViewAttribution.trim()) ? stripMarkdown(q.modernView) : "",
          modernViewAttribution: (typeof q.modernView === "string" && typeof q.modernViewAttribution === "string" && q.modernView.trim() && q.modernViewAttribution.trim()) ? stripMarkdown(q.modernViewAttribution) : ""
        });
      });
      ...
      return {
        ...
        error: error.name === "AbortError"
          ...
          : "Impossible de générer les questions (erreur technique)."
      };
    }
  }
}
...
// Pré-chargement en arrière-plan : on lance la génération Gemini dès l'ouverture de l'app (et dès
// qu'un quiz est terminé), pour que le prochain clic sur "Les gestes du quotidien" soit instantané
// la plupart du temps au lieu d'attendre l'appel réseau. On ne relance jamais un pré-chargement
// tant que le lot en cours n'a pas été consommé (joué) ou n'est pas déjà en cours de récupération.
// (quizPrefetchResult / quizPrefetchPromise sont déclarées tout en haut du script, voir plus haut.)
function quizStartPrefetch(){
  if(quizPrefetchPromise || quizPrefetchResult) return;
  quizPrefetchPromise = quizGenerateAIQuestions().then(result => {
    quizPrefetchResult = result;
    quizPrefetchPromise = null;
    return result;
  });
}

async function quizGetSessionQuestions(){
  if(quizPrefetchResult){
    const r = quizPrefetchResult;
    quizPrefetchResult = null;
    return r;
  }
  if(!quizPrefetchPromise) quizStartPrefetch();
  const r = await quizPrefetchPromise;
  quizPrefetchResult = null;
  return r;
}
...
function initQuizList(){
  ...
  // Ouverture d'un chapitre : Gemini génère 10 questions inédites à chaque lancement. La banque
  // fixe (QUIZ_CHAPTERS) ne sert que de secours si l'IA échoue (pas de clé, quota, réseau), pour
  // que le quiz reste toujours utilisable même quand Gemini est indisponible.
  const onClick = async (e) => {
    const card = e.target.closest(".quiz-chapter");
    if(!card || card.classList.contains("soon") || quizGenerating) return;
    const chapterId = card.getAttribute("data-id");
    const chapter = quizGetChapter(chapterId);

    quizGenerating = true;
    const readyImmediately = !!quizPrefetchResult;
    if(!readyImmediately && statusEl){
      statusEl.style.display = "block";
      statusEl.textContent = "Génération des questions avec Gemini...";
    }

    const result = await quizGetSessionQuestions();
    ...
    if(result.success){
      ...
      if(statusEl){
        statusEl.textContent = result.error + " Questions de secours utilisées à la place.";
      }
...
function quizRenderQuestion(){
  ...
  document.getElementById("quizModernView").style.display = "none";
...
function initQuizPlay(){
  ...
  const onOptionClick = (e) => {
    ...
    // Bloc "avis contemporain" : ne s'affiche que si la question porte un vrai avis plus souple,
    // documenté et attribué à un décisionnaire nommé. Il ne remplace jamais le badge Permis/Interdit
    // (qui reste la réponse notée) — il signale juste qu'un débat existe sur ce point précis.
    const modernEl = document.getElementById("quizModernView");
    if(q.modernView && q.modernViewAttribution){
      modernEl.innerHTML = "";
      modernEl.appendChild(document.createTextNode(q.modernView));
      const attribSpan = document.createElement("span");
      attribSpan.className = "quiz-modern-view-attrib";
      attribSpan.textContent = q.modernViewAttribution;
      modernEl.appendChild(attribSpan);
      modernEl.style.display = "block";
    } else {
  ...
  const onNext = () => {
    ...
    if(quizCurrentIndex >= quizSessionQuestions.length - 1){
      ...
      quizFinished = true;
      quizStartPrefetch(); // le prochain quiz commence déjà à se préparer en arrière-plan
